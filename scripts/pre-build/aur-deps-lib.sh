...
install_aur_dep() {
    ...
    local base dir logfile clone_ok=0
    ...
    if ! ( cd "$dir" && makepkg -s --nocheck --noconfirm --needed --skippgpcheck >"$logfile" 2>&1 ); then
        echo "  [hook] ✗ makepkg failed for AUR dependency $pkg ($base)" >&2
    ...
    if [ "${#built[@]}" -eq 0 ]; then
        echo "  [hook] ✗ AUR dependency $pkg produced no package files" >&2
        exit 1
    fi

    sudo pacman -U --noconfirm --needed "${built[@]}" >"$AUR_DEPS_DIR/pacman-u-${base}.log" 2>&1 || {
        echo "  [hook] ✗ pacman -U failed for AUR dependency $pkg" >&2
    ...
    echo "  [hook] ✓ Installed AUR dependency: $pkg ($base)"
}
